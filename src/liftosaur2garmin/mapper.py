...
# Custom user-defined mappings (loaded from ~/.liftosaur2garmin/custom_mappings.json)
...
def _lookup_candidates(name: str) -> list[str]:
    ...
    if ", " in name:
        base, equipment = [part.strip() for part in name.rsplit(", ", 1)]
        mapped_equipment = {
            "Barbell": "Barbell",
        ...
        for candidate in (f"{base} ({mapped_equipment})", base):
